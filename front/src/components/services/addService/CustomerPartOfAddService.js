...
const CustomerPartOfAddService = props => {
  const { loading, error, data } = useQuery(getCustomerByAllQuery, {
    variables: {
  ...
  if (error) {
    return (
      <h1 className="customerDetail__error">
        Znajdź tego klienta i dodaj mu serwis. Coś poszło nie tak
      </h1>
    );
  }
  ...
  return (
    <Fragment>
      <h1 className="customerDetail__header">Dane klienta</h1>
      <form className="customerDetail__form">
        <label className="customerDetail__label" htmlFor="name">
          Imię i Nazwisko
        </label>
        <input
        ...
        />
        <label className="customerDetail__label" htmlFor="mail">
          Kontakt mailowy
        </label>
        <input
...
export default connect(null, mapDispatchToProps)(CustomerPartOfAddService);
